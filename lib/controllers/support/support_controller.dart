...
for (var i = 0; i < uploadImages.length; i++) {
  ...
  formData.files.add(
    MapEntry("files[$i]", await MultipartFile.fromFile(file.path, filename: fileName))
   );
}
  ...
  Future getSupport(String id) async {
   ...
   var request = {
    ...
    var response =
        await DioClient().post(ApiLinks.getSupport, request).catchError((error) {
      if (error is BadRequestException) {
  ...
  Future getSupportDetails(String id) async {
   ...
   var request = {
    ...
    var response =
        await DioClient().post(ApiLinks.getSupportDetails, request).catchError((error) {
      if (error is BadRequestException) {
  ...
  //  isLoadingSupportDetails.value = true;
   var request = {
    ...
    var response =
        await DioClient().post(ApiLinks.supportReply, request).catchError((error) {
          debugPrint(error.toString());
    //   if (error is BadRequestException) {
    //     isLoadingSupportDetails.value = false;
    //      var apiError = json.decode(error.message!);
