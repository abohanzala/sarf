...
class InvoiceMembersHomeListScreen extends StatefulWidget {
  ...
  const InvoiceMembersHomeListScreen({super.key, required this.expanseId, required this.budgetId});
    ...
    search(value) {
      FocusScope.of(context).unfocus();
        ...
        if(value.isEmpty){
            ...
            cityList = ctr.getInvoiceMemberListHome('',widget.expanseId,widget.budgetId);
            cityList?.then((value){
          ...
          cityList = ctr.getInvoiceMemberListHome( searchValue.text,widget.expanseId,widget.budgetId);
          cityList?.then((value){
               var receive = value?.data?.fold(0, (previousValue, element) => previousValue +(element.invoicesCount ?? 0) );
              var total = value?.data?.fold(0, (previousValue, element) => previousValue +(element.invoicesSumAmount ?? 0) );
              var mem = value?.data?.fold(0, (previousValue, element) => previousValue +(element.invoicesCount! > 0 ? 1 : 0) );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:  Column(
          ...
          children: [
            Container(
          padding: EdgeInsets.only(left: kIsWeb == true ? Get.width * 0.11 : 16.w, top: 20.h, right: kIsWeb == true ? Get.width * 0.11 : 16.w),
          height: 120.h,
